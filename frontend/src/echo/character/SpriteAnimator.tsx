...
import { ANIMATIONS, SPRITE_FRAME_W, SPRITE_FRAME_H, SPRITE_SHEET_COLS } from '../core/types'
  ...
  useEffect(() => {
    ...
    function draw(now: number) {
      ...
      if (now - lastTimeRef.current >= interval) {
        ...
        if (facingLeft) { ctx.save(); ctx.translate(displaySize, 0); ctx.scale(-1, 1) }
        if (imgRef.current) {
          ctx.drawImage(imgRef.current, sx, sy, SPRITE_FRAME_W, SPRITE_FRAME_H, 0, 0, displaySize, displaySize)
        }
    ...
    <>
      ...
      <canvas
        ...
        style={{
          ...
          animation:
            state === 'JUMPING' ? `echoJump ${jumpDuration} ease-in-out` :
            state === 'SQUISH'  ? `echoSquish ${squishDuration} ease-in-out` :
            'none',
        }}
      />
    </>
